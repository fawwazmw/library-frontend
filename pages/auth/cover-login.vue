...
<script lang="ts" setup>
    ...
    const tokenCookie = useCookie('auth_token'); // Menggunakan cookie untuk token
    ...
    const handleLogin = async () => {
        try {
            ...
            if (response.ok && data.token) {
                ...
                // Arahkan ke /dashboard yang akan membuka pages/dashboard/index.vue
                router.push('/');
            } else {
    ...
    onMounted(() => {
        if (tokenCookie.value) {
            // Arahkan ke dashboard jika pengguna sudah login berdasarkan token di cookie
            router.push('/');
        }
    });
</script>
